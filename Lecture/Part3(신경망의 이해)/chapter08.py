...
# 1) 환경 변수 지정: 환결 변수에는 입력 값과 타길 결괏값이 포함된 데이터셋, 합습률 등이 포함됩니다.
#                 또한, 활성화 함수와 가중치 등도 선언되어야 합니다.
# 2) 신경망 실행: 초깃값을 입력하여 활성화 함수와 가충치를 거쳐 결괏값이 나오게 합니다.
# 3) 결과를 실제 값과 비교: 오차를 측정합니다.
# 4) 역전파 실행: 출력층과 은닉층의 가중치를 수정합니다.
# 5) 결과 출력


# 부록 B "심화학습 2: 파이썬 고드로 확인하는 신경망" 참조. 273p
